...
$CPASS=$_POST['Pass'];
$PASS2=$_POST['Pass2'];
$query="SELECT PASSWORD FROM teachermas WHERE teacherid='$UID'";
...
if($data['PASSWORD']==$CPASS){
    ...
    if($PASS1==$PASS2){
         if($CPASS!=$PASS1){
                $query="UPDATE teachermas set PASSWORD='$PASS1' WHERE teacherid='$UID'";
                $result=mysqli_query($link,$query);
                if(mysqli_affected_rows($link)>0){
                    $_SESSION['check']=true;
...
<div class="form-group">
  ...
  <div class="col-md-8">
   ...
   <a href="s_teacher.php"></a> <input type="Button"  class="btn btn-info" value="Cancel" /></a>
  </div>
</div>
