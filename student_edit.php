...
if(isset($_POST['Drollno']) || isset($_SESSION['Usid'])){

if(isset($_POST['Drollno']))
 $_SESSION['Usid']=$_POST['Drollno'];
 if(!isset($_POST['Drollno']))
 $_POST['Drollno']=$_SESSION['Usid'];
?>
                    ...
                    <div class="panel panel-primary" >
                        <div class="bgcolor">
                            ...
                            <input type="button" onclick="statechanged()" class="btn btn-primary myhomebtn" style="float: right; margin-right:30px;margin-top: 12px; " value="  Edit  "/>
                                <h3 ><?php echo "$_POST[Drollno]'s"; ?> Profile</h3>
                             </div>
                        </div>
...
$query="SELECT FNAME,MNAME,LNAME,MOBNO,EMAIL,FATHERNAME,ADDRESS,CITY,STATE,DOB,GENDER,PHOTO,STATEID,CITYID FROM studentdetail WHERE rollno='$_POST[Drollno]'";
...
if(mysqli_affected_rows($link)>0){
    ...
    $query="SELECT ACDACHVMT,SPORTS,CULTURAL,OTHERS,INTER_BOARD,INTER_YEAR,INTER_ROLLNO,HIGHSCHOOL_BOARD,HIGHSCHOOL_YEAR,HIGHSCHOOL_ROLLNO FROM studentacad WHERE rollno='$_POST[Drollno]'";
    $result=mysqli_query($link,$query);
    if(mysqli_affected_rows($link)>0){
$data=mysqli_fetch_array($result,MYSQLI_ASSOC);
                                    ...
                                    <td width="70% " >
                                        <fieldset><br />
                                              <div class="form-group">
                                                <label class="col-md-4 control-label" >Roll no <label style="color: red;" > * </label> :</label>   
                                            
                                                <div class="col-md-7">
                                                    <input  type="text" disabled="" value="<?php echo $_POST['Drollno']?>" placeholder="Roll Number" class="form-control input-md" required="">
                                                    <input type="hidden" name="Usid" value="<?php echo $_POST['Drollno']?>"/>
                                                </div>
                                            </div>                           
                                            <div class="form-group">
    ...
    bootbox.confirm("Are you sure to submit this form?",function(res){
       if(res==true){
          //  phpForm.action="stueditprofile.php";
            //phpForm.submit();
           $.post("student_edit1.php",$('#phpForm').serialize(),
            function(res){
                if(true){
