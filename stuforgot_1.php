...
$query="SELECT email FROM studentdetail WHERE rollno='$UID'";
$result=mysqli_query($link,$query);
if(mysqli_affected_rows($link)>0){
    $data=mysqli_fetch_array($result,MYSQLI_ASSOC);
    $email=$_POST['Email'];
    if($data['email']==$email){
        $query="SELECT uuid FROM loginmas WHERE userid='$UID'";
        $result=mysqli_query($link,$query);
        $data=mysqli_fetch_array($result,MYSQLI_ASSOC);
        $uuid=$data['uuid'];
        $to=$email;
        
        $link="<a href='http://localhost:80//project/schangepassword.php?email=$to&usid=$UID&uuid=$uuid'>Click Here</a>";
        $message="Dear Student,<br /><br /><br />Click the below link to change your password :<br>";
        $message.=$link;
...
<meta name="viewport" content="width=device-width,initial-scale=1" />
	<title>Student_ChangePassword</title>
...
<div class="form-group">
  ...
  <div class="col-md-5">&nbsp;&nbsp;&nbsp;&nbsp;<a href="stuforgot2.php" style="font-size: 17px;">Try another way for <b>Sign-in</b></a></div>
</fieldset>
</form>
        </div></div>
    <div class="col-md-4">
