...
$query="SELECT TEACHERID,PASSWORD FROM teachermas where teacherid='$UID'";
$result=mysqli_query($link,$query);
if(mysqli_affected_rows($link)>0){
 
 $data=mysqli_fetch_array($result,MYSQLI_ASSOC);
 $DBPASS=$data['PASSWORD'];
$query="SELECT FNAME,LNAME FROM teacherdetail where teacherid='$UID'";
$result=mysqli_query($link,$query);

if(mysqli_affected_rows($link)>0){
    $data=mysqli_fetch_array($result,MYSQLI_ASSOC);
    if($PASSWORD==$DBPASS){
        $_SESSION['utname']=$data['FNAME'];
        $_SESSION['utid']=$UID;
        header("location:s_teacher.php");
    }
    else{
                    $i=1;      
    }
...
<div class="form-group">
  ...
  <div class="input-group">
   <span class="input-group-addon"> <span class="glyphicon glyphicon-lock"></span></span>
    ...
    <a href="tcforgot1.php" style="float: right;">Forgot Password?</a></span>
   </div>
<div class="row">
...
if(isset($_POST['login'])){
    if($i==1){
        echo '<script language="javascript">';
                    ...
                    echo 'bootbox.alert("<div class=errormessage><br>1This userid does not exists</div>")';
                    echo '</script>';
        }
}
?>
