...
if(isset($_POST['Submit'])){
    ...
    if($pass1==$pass2){
        ...
        $uuid=$data['uuid()'];
        $query="update loginmas set password='$pass1',uuid='$uuid' where userid='$_SESSION[usid]'";
        $result=mysqli_query($link,$query);
        ...
        $str="<div class=errormessage><br />You have already used that password<br />please try a different one...!!</div>";
        }
    }
    else{
...
{
    $i=0;
if(!isset($_REQUEST['email']) || !isset($_REQUEST['uuid']) || !isset($_REQUEST['usid'])){
    echo "<br /><br /><center><h1>Session has expired....</h1><h3>Please wait you are re-directing in few seconds</center>";
    echo "<h3><center>If you don't redirect automatically, Please <a href=studentlogin.php>Click Here</a> to redirect yourself...</center>";
    die();
}
?>
...
$uuid=$_REQUEST['uuid'];
$rollno=$_REQUEST['usid'];
$i=0;
$query="select rollno from studentdetail where rollno='$rollno'";
$result=mysqli_query($link,$query);
if(mysqli_affected_rows($link)!=1){
    ...
    $data=mysqli_fetch_array($result,MYSQLI_ASSOC);
     $rollno=$data['rollno'];
    $query="select email from studentdetail where rollno='$rollno'";
    $result=mysqli_query($link,$query);
     if(mysqli_affected_rows($link)==1){
        $data=mysqli_fetch_array($result,MYSQLI_ASSOC);
        if($email==$data['email']){
            $query="select uuid from loginmas where userid='$rollno'";
            $result=mysqli_query($link,$query);
            if(mysqli_affected_rows($link)==1){
             $data=mysqli_fetch_array($result,MYSQLI_ASSOC);
                if($uuid==$data['uuid']){
                    
                    $_SESSION['usid']=$rollno;
            ?>
